...
def main():
    ...
    try:
        if args.preset:
            ...
            # Resolve output into generated_csv_data/ subfolder with timestamped set
            d = os.path.dirname(os.path.abspath(__file__))
            generated_csv_dir = os.path.join(os.path.dirname(d), "generated_csv_data")
            
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            set_folder_name = f"production_set_{timestamp}"
            set_dir = os.path.join(generated_csv_dir, set_folder_name)
            os.makedirs(set_dir, exist_ok=True)
            
            print(f"Creating massive files in: generated_csv_data/{set_folder_name}/")
