...
from app.schemas.dataset import DatasetList, DatasetResponse
...
def upload_dataset(
    ...
    if not filename:
        raise HTTPException(
            status_code=400, detail="Filename is required. Please provide a valid file."
        )
    ...
    try:
        ...
        os.remove(file_path)
        raise HTTPException(
            status_code=400, detail=f"Invalid {ext.upper()} format: {str(e)}"
        )
    except Exception as e:
        os.remove(file_path)
        raise HTTPException(
            status_code=400, detail=f"Unexpected error while parsing file: {str(e)}"
        )
