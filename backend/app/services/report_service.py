...
def generate_report(dataset_id: int, db: Session) -> dict:
    ...
    for res in failed_results:
        rule_name = (
            rules[res.rule_id].name if res.rule_id in rules else f"Rule {res.rule_id}"
        )
        if rule_name not in failure_patterns:
            failure_patterns[rule_name] = {
                "rule_name": rule_name,
                "count": 0,
                "failed_rows": 0,
            }
    ...
    top_patterns = sorted(
        failure_patterns.values(), key=lambda x: x["failed_rows"], reverse=True
    )[:10]

    recommendations = []
    for pattern in top_patterns:
        recommendations.append(
            f"Review rows failing '{pattern['rule_name']}' which affected {pattern['failed_rows']} rows."
        )

    executive_summary = {
        "status": "Excellent"
        if latest_score.score >= 90
        else "Good"
...
def get_trend_data(
    ...
    query = db.query(QualityScore).filter(
        QualityScore.checked_at >= start_date, QualityScore.checked_at <= end_date
    )
