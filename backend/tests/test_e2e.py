...
class TestE2EHappyPath:
    def test_full_flow_clean_data(self, client):
        ...
        # Upload clean CSV
        ds = client.post("/api/datasets/upload", files=csv_file(CLEAN_CSV, "e2e_clean.csv"), headers=headers).json()
...
class TestE2EDirtyData:
    def test_full_flow_dirty_data_low_score(self, client):
        ...
        ds = client.post("/api/datasets/upload", files=csv_file(DIRTY_CSV, "e2e_dirty.csv"), headers=headers).json()
...
class TestE2EMultipleDatasets:
    def test_datasets_scored_independently(self, client):
        ...
        ds1 = client.post("/api/datasets/upload", files=csv_file(CLEAN_CSV, "iso_clean.csv"), headers=headers).json()
        ds2 = client.post("/api/datasets/upload", files=csv_file(DIRTY_CSV, "iso_dirty.csv"), headers=headers).json()
...
class TestE2ERuleUpdateAndDelete:
    def test_update_rule_then_recheck(self, client):
        ...
        ds = client.post("/api/datasets/upload", files=csv_file(CLEAN_CSV, "e2e_update.csv"), headers=headers).json()
        rule = client.post("/api/rules", json=not_null_rule("name", "HIGH"), headers=headers).json()
        rule_id = rule["id"]

        # Update severity to LOW (using PATCH since PUT isn't supported)
        updated = client.patch(f"/api/rules/{rule_id}", json={"severity": "LOW"}, headers=headers)
    ...
    def test_delete_rule_removes_from_list(self, client):
        ...
        rule = client.post("/api/rules", json=not_null_rule("deleteme_field", "HIGH"), headers=headers).json()
        rule_id = rule["id"]
...
class TestE2EHealthAndSmoke:
    ...
    def test_trends_populated_after_checks(self, client):
        ...
        ds = client.post("/api/datasets/upload", files=csv_file(CLEAN_CSV, "trend_e2e.csv"), headers=headers).json()
        client.post("/api/rules", json=not_null_rule("name", "HIGH"), headers=headers)
