...
class TestCreateRule:
    ...
    def test_create_not_null_rule(self, client, auth_token):
        ...
        headers = {"Authorization": f"Bearer {auth_token}"}
        resp = client.post(
            "/api/rules", json=not_null_rule("email", "HIGH"), headers=headers
        )
    ...
    def test_create_data_type_rule(self, client, auth_token):
        ...
        headers = {"Authorization": f"Bearer {auth_token}"}
        resp = client.post(
            "/api/rules", json=data_type_rule("age", "int", "MEDIUM"), headers=headers
        )
        assert resp.status_code == 201
        data = resp.json()
    ...
    def test_create_range_rule(self, client, auth_token):
        ...
        headers = {"Authorization": f"Bearer {auth_token}"}
        resp = client.post(
            "/api/rules", json=range_rule("score", 0, 100, "MEDIUM"), headers=headers
        )
        assert resp.status_code == 201
        data = resp.json()
    ...
    def test_create_unique_rule(self, client, auth_token):
        ...
        headers = {"Authorization": f"Bearer {auth_token}"}
        resp = client.post(
            "/api/rules", json=unique_rule("id", "HIGH"), headers=headers
        )
        assert resp.status_code == 201
    ...
    def test_create_rule_missing_required_fields(self, client, auth_token):
        ...
        resp = client.post(
            "/api/rules",
            json={"name": "incomplete", "dataset_type": "csv"},
            headers=headers,
        )
...
class TestUpdateRule:
    ...
    def test_update_rule_as_non_owner(self, client, auth_token):
        ...
        res = client.post(
            "/api/auth/register",
            json={
                "email": f"other_{uuid.uuid4().hex}@req.com",
...
class TestDeleteRule:
    ...
    def test_delete_rule_as_non_owner(self, client, auth_token):
        ...
        res = client.post(
            "/api/auth/register",
            json={
                "email": f"another_{uuid.uuid4().hex}@req.com",
                "password": "Password123",
                "full_name": "Other2",
            },
        )
